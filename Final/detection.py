...
while True:
    ...
    cv2.rectangle(frame, (0,0),(2*imW//5,imH),(0,0,0),2)
    cv2.rectangle(frame, (2*imW//5,0),(3*imW//5,imH),(0,0,0),2)
    cv2.rectangle(frame, (3*imW//5,0),(imW,imH),(0,0,0),2)
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) and labels[int(classes[i])].upper() == "PERSON"):
            ...
            center = (((xmin+xmax)//2),((ymin+ymax//2))) #Tuple holding Center point of object in frame 
            ...
            if (xmin+xmax)//2 >= (2*imW//5) and (xmin+xmax)//2 <= (3*imW//5):
                #print("GO STRAIGHT!")
                os.system('python3 servo_straight.py')
                
            if (xmin+xmax)//2 > (3*imW//5):
                #print("GO RIGHT")
